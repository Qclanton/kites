...
// Options natsort start

foreach ($this->data['options'] as $oid=>$option) {
$sopt=array();
    ...
    $old_options=array();
				...
				foreach($sopt as $key=>$val) {
				
			if (preg_match("([^0-9])",$val,$ok)){
				
				$ks=array_search($val, $osopt);
				$old_options[$ks]=$option['option_values'][$ks];
				
				
			}	 else {
				
				$new_options[]=$option['option_values'][$key]; 
	...
	foreach ($old_options as $ols) {
		$new_options[]=$ols;		
	}
	
	
				
      $this->data['options'][$oid]['option_values']=$new_options;
