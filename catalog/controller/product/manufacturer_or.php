		...
		if ($manufacturer_info) {
			...
			foreach ($results as $result) {
				...
				$this->data['products'][] = array(
					...
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => mb_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, 180,'UTF-8') . '..',
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
			...
			$this->data['sorts'][] = array(
				...
				'href'  => $this->url->link('product/manufacturer/product', 'manufacturer_id=' . $this->request->get['manufacturer_id'] . '&sort=p.sort_order&order=DESC' . $url)
			);
			...
			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_count_rating_asc'),
				'value' => 'count_rating-DESC',
				'href'  => $this->url->link('product/manufacturer/product', 'manufacturer_id=' . $this->request->get['manufacturer_id'] . '&sort=count_rating&order=DESC' . $url)
			);	
			
			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_new_desc'),
				'value' => 'p.date_added-DESC',
				'href'  => $this->url->link('product/manufacturer/product', 'manufacturer_id=' . $this->request->get['manufacturer_id'] . '&sort=p.date_added&order=DESC' . $url)
			);
