...
class ControllerAccountSimpleRegister extends Controller {
    ...
    private $language_code = '';
  	...
  	public function index() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $data = array(
                'firstname' => $this->get_total_value('firstname'),
                'lastname' => $this->get_total_value('lastname'),
                'email' => $this->get_value('email'),
                'password' => trim($this->request->post['password']),
                'telephone' => $this->get_total_value('telephone'),
                'fax' => $this->get_total_value('fax'),
                'newsletter' => isset($this->request->post['subscribe']) ? $this->request->post['subscribe'] : ($this->data['simple_registration_subscribe'] == 1),
                'company' => $this->get_total_value('company'),
                'company_id' => $this->get_total_value('company_id'),
                'tax_id' => $this->get_total_value('tax_id'),
                'address_1' => $this->get_total_value('address_1'),
                'address_2' => $this->get_total_value('address_2'),
                'postcode' => $this->get_total_value('postcode'),
                'city' => $this->get_total_value('city'),
    ...
    private function load_fields() {
        if (empty($this->data['customer_fields'])) {
            ...
            if (!empty($set['registration'])) {
                ...
                foreach ($this->data['customer_fields'] as $id => $settings) {
                    if (strpos($id, 'main_') == 0 && !array_key_exists($id, $this->data['display_customer_fields'])) {
                        $tmp = $settings;
                        $tmp['type'] = 'hidden';
                        $this->data['display_customer_fields'][$id] = $tmp;
                    }
                }
            }
        }
    ...
    private function get_total_value($id) {
        $this->load_fields();
        $value = null;
