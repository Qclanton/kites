	...
	public function update() {
		...
		if (isset($this->request->post['product_id'])) {
			...
			if ($product_info) {						
				...
				$accessories = $this->request->post['accessories'];
			} else {
				$accessories = array();	
			}
			...
			foreach ($product_accessories as $product_accessory) {
				...
				// options
				if (isset($this->request->post['accessory_option'][$product_accessory['product_id']]) && !empty($accessories[$product_accessory['product_id']])) {
					$accessory_option[$product_accessory['product_id']] = array_filter($this->request->post['accessory_option'][$product_accessory['product_id']]);
				} else {
					$accessory_option[$product_accessory['product_id']] = array();	
				}
				
				$product_accessory_options = $this->model_catalog_product->getProductOptions($product_accessory['product_id']);
				
				foreach ($product_accessory_options as $product_accessory_option) {
					if ($product_accessory_option['required'] && empty($accessory_option[$product_accessory['product_id']][$product_accessory_option['product_option_id']]) && !empty($accessories[$product_accessory['product_id']])) {
						$json['error']['accessory_option'][$product_accessory['product_id'] . "-" . $product_accessory_option['product_option_id']] = sprintf($this->language->get('error_required'), $product_accessory_option['name']);
					}
				}				
				// end options
			}
			...
			if (!isset($json['error'])) {
				...
				foreach ($this->request->post['accessories'] as $accessory_id=>$accessory_quantity){
					if((int)$accessory_quantity){
						$this->cart->add($accessory_id, $accessory_quantity, $accessory_option[$accessory_id]);
					}
				}
			}
