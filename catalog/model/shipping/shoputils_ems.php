...
class ModelShippingShoputilsEMS extends Model {
    ...
    private function getRateDescription($rate, $address){
        ...
        if ($this->config->get('shoputils_ems_show_advanced')) {
            ...
            $total1 = $this->cart->getSubTotal();
			$free_shipping = (float)$this->config->get('shoputils_ems_free_shipping');
			if (!empty($free_shipping) && ($total1 > $free_shipping) && ($this->session->data['ems_item_id'] == 'city--moskva' || $this->session->data['ems_item_id'] == 'city--sankt-peterburg')) {
				//$total =0;
				$total = $this->currency->convert(0, self::EMS_CURRENCY_CODE, $this->currency->getCode());
			} else {
    ...
    public function getQuotes($address){
        ...
        $free_shipping = (float)$this->config->get('shoputils_ems_free_shipping');
        /*if (!empty($free_shipping) && ($total > $free_shipping) && ($this->session->data['ems_item_id'] == 'city--moskva' || $this->session->data['ems_item_id'] == 'city--sankt-peterburg')) {
       ...
       // } else {
			...
			if(!empty($free_shipping) && ($total > $free_shipping) && ($this->session->data['ems_item_id'] == 'city--moskva' || $this->session->data['ems_item_id'] == 'city--sankt-peterburg')) {
			//$cost = 0;
			$cost = $this->currency->convert(0, self::EMS_CURRENCY_CODE, $this->currency->getCode());
			} else {
