      ...
      $(widget).bind("tweet:load", function(){
        $.getJSON(build_api_url(), function(data){
          ...
          list.append($.map(tweets, function(o) {return "<li>" + t(s.template, o) + "</li>";}).join('')).
            ...
            {
                var $selr = $(".tweet_list li");
                var $items = $selr.size();
                var $item_len = $selr.width();
                var $curr_pos = parseInt($(".tweet_list").css('left'));
                
                var $left = ($curr_pos-$item_len <= ((-1)*($items*$item_len))) ? 0 : $curr_pos - $item_len;

                $(".tweet_list").fadeOut('slow').animate({ left : $left }).fadeIn('slow');
                $tweet_timeout = setTimeout($.TwitterCarousel,10000);
